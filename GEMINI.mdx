...
keywords:
  ...
  - Astrology
description: >-
  Comprehensive guide on GEMINI. Learn about its significance, effects, and
...
-   **`locale`**: Target locale code (`en`, `hi`, `ne`). Default `en`.
-   **`audioUrl`**: (Optional) CDN link to synthesized audio summary.

#### b. Article Body & AI BLUF Summaries
-   **AI BLUF Summaries (`<AIBlufSummary>`)**: Every major section (especially `## Introduction` and primary thematic sections) MUST begin with an `<AIBlufSummary>...</AIBlufSummary>` block containing a clear, 2-3 sentence bottom-line summary for rapid AI citation and TTS voice generation.
...
-   **Introduction (`# Introduction...` / `## Introduction...`)**: Start with a captivating introduction that defines the topic and explains its importance to the reader.
...
3.  **BLUF Summaries for Audio/AI**: Ensure every `<AIBlufSummary>` is concise, punchy, and self-contained for optimal TTS voice synthesis.
4.  **SEO is Paramount**: Naturally weave keywords into the title, headings, and body content. Ensure the `description` and `title` are optimized for search engine results pages (SERPs).
